...
class OnlineConsultationController extends Controller
{
    ...
    public function cancelReservation(Request $request, $id)
    {
        ...
        if (!Hash::check($request->password, Auth::user()->password)) {
            return redirect()->back()->withErrors(['password' => 'Password otorisasi salah.']);
        }
        ...
        ReservationLog::create([
            'reservation_id' => $reservation->id,
            'user_id' => Auth::id(), // ID admin yang membatalkan
            'reason' => $request->reason,
        ]);

        return redirect()->back()->with('success', 'Reservasi berhasil dibatalkan dan dicatat dalam log.');
    }
