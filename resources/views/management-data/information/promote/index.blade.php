        ...
        <div class='dashboard-content'>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <form method="GET" action="{{ route('information.promote.index') }}">
                            <div class="input-group mb-3">
                                <!-- Dropdown Kategori -->
                                <select name="flag" id="flag" class="form-select me-2"
                                    onchange="this.form.submit()">
                                    <option value="" {{ request('flag') == '' ? 'selected' : '' }}>Semua Kategori
                                    </option>
                                    <option value="Diskon" {{ request('flag') == 'Diskon' ? 'selected' : '' }}>Diskon
                                    </option>
            ...
            <div class="row cards-container">
                @forelse($promotions as $promotion)
                    <div class="col-md-3 mb-4 promo-card" data-title="{{ $promotion->title }}"
                        data-description="{{ $promotion->description }}"
                        data-image="{{ $promotion->media->first() ? $promotion->media->first()->file_url : asset('images/default-image.jpg') }}">
                        <div class="card" style="border-radius: 12px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                            <img class="card-img-top"
                                ...
                                alt="Promotion Image" style="width: 100%; height: 450px; object-fit: cover;">
                            <div class="card-body" style="height: 100px; object-fit: cover;">
                                <p class="card-text">{{ $promotion->flag }}</p>
                                <h5 class="card-title">{{ $promotion->title }}</h5>
                            </div>
                            <div class="card-footer d-flex justify-content-between">
                                <a href="{{ route('information.promote.edit', $promotion->id) }}"
                                    class="btn btn-sm btn-success custom-btn">
                                    Edit
                                </a>
                                <form action="{{ route('information.promote.destroy', $promotion->id) }}" method="POST"
                                ...
                                @if (!$promotion->is_published)
                                    ...
                                    <form action="{{ route('information.promote.publish', $promotion->id) }}"
                                        method="POST" onsubmit="return confirm('Yakin ingin mempublikasikan promo ini?')">
                                        @csrf
                                        @method('PUT')
                                ...
                                @else
                                    <!-- Tombol untuk Draft -->
                                    <form action="{{ route('information.promote.draft', $promotion->id) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin memindahkan promo ke Draft?')">
                                        @csrf
                                        @method('PUT')
...
$(document).ready(function() {
    ...
    $(".menu-toggle").click(function() {
        if (mobileScreen.matches) {
            ...
            // Klik pada gambar untuk memunculkan modal
            $('.promo-card img').on('click', function(event) {
                event.stopPropagation(); // Hindari trigger event lainnya
                const imageUrl = $(this).closest('.promo-card').data('image');
